 ...
 */
package quests.Q19908_LevelUpTo85;
 ...
 * @author Magik
 */
public class Q19908_LevelUpTo85 extends Quest
{
	private static final int QUEST_ID = 19908;
	
	public Q19908_LevelUpTo85()
	{
		super(QUEST_ID);
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			...
			case "COMPLETE":
			{
				...
				final QuestState nextQuestState = player.getQuestState(Q10381_StopSelMahumTroops5.class.getSimpleName());
				if (nextQuestState == null)
				{
					player.sendPacket(new ExQuestDialog(10381, QuestDialogType.ACCEPT));
				}
	...
	public void onPlayerLevelChange(OnPlayerLevelChanged event)
	{
		...
		else if ((questState != null) && questState.isStarted() && !questState.isCompleted() && (player.getLevel() == 85))
		{
			questState.setCount(getQuestData().getGoal().getCount());
			questState.setCond(QuestCondType.DONE);
			player.sendPacket(new ExQuestNotification(questState));
